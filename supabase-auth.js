(() => {
  ...
  document.getElementById('loginForm').onsubmit = async (e) => { e.preventDefault(); const email=loginId.value.trim(), password=e.target.querySelector('input[type=password]').value; const {error:e1}=await client.auth.signInWithPassword({email,password}); e1?error('이메일 또는 비밀번호를 확인해 주세요.'):enter(); };
  document.getElementById('signupForm').onsubmit = async (e) => { e.preventDefault(); const email=signupId.value.trim(), password=e.target.querySelector('input[type=password]').value, nickname=document.getElementById('nickname').value.trim(); const {error:e1}=await client.auth.signUp({email,password,options:{data:{nickname}}}); if(e1)return error(e1.message); localStorage.setItem('badayaPendingProfile',JSON.stringify({nickname})); showAuth('signupStep2'); };
